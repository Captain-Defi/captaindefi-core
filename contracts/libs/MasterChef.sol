...
import "./CaptainToken.sol";

// MasterChef is the master of Captain. He can make Captain and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Captain is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CAPDs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accCaptainPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accCaptainPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. CAPD to distribute per block.
        uint256 lastRewardBlock;  // Last block number that CAPD distribution occurs.
        uint256 accCaptainPerShare;   // Accumulated CAPD per share, times 1e12. See below.
    ...
    // The CAPD TOKEN!
    CaptainToken public captain;
    // Dev address.
    address public devaddr;
    address private devaddrPreventive;
    // Block number when bonus CAPD period ends.
    uint256 public bonusEndBlock;
    // CAPD tokens created per block.
    uint256 public captainPerBlock;
    // Deposit Fee address
    ...
    // The block number when CAPD mining starts.
    ...
    constructor(
        CaptainToken _captain,
        address _devaddr,
        address _devaddrPreventive,
        address _feeAddress,
        uint256 _captainPerBlock,
        uint256 _startBlock
    ) public {
        captain = _captain;
        devaddr = _devaddr;
        ...
        captainPerBlock = _captainPerBlock;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
        ...
        poolInfo.push(PoolInfo({
            ...
            accCaptainPerShare: 0,
            depositFeeBP: _depositFeeBP
        }));
    }

    // Update the given pool's CAPD allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending CAPDs on frontend.
    function pendingCaptain(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accCaptainPerShare = pool.accCaptainPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 captainReward = multiplier.mul(captainPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accCaptainPerShare = accCaptainPerShare.add(captainReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accCaptainPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 captainReward = multiplier.mul(captainPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        captain.mint(devaddr, captainReward.div(5));
        captain.mint(devaddrPreventive, captainReward.div(5));
        captain.mint(address(this), captainReward);
        pool.accCaptainPerShare = pool.accCaptainPerShare.add(captainReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for CAPD allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCaptainPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeCaptainTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accCaptainPerShare).div(1e12);
    ...
    // Safe captain transfer function, just in case if rounding error causes pool to not have enough CAPDs.
    function safeCaptainTransfer(address _to, uint256 _amount) internal {
        uint256 captainBal = captain.balanceOf(address(this));
        if (_amount > captainBal) {
            captain.transfer(_to, captainBal);
        } else {
            captain.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _captainPerBlock) public onlyOwner {
        captainPerBlock = _captainPerBlock;
    }
